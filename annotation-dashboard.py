...
if use_sample:
    raw = pd.DataFrame([{
        ...
        "작업 수정 시간":120,"CO 모니터링 URL":"http://example.com"
    } for _ in range(200)])
elif uploaded:
...
# 기간 필터 (open_date to target_end_date)
...
weekly["review_wait"]   = df[df["review_date"].isna()].groupby("week_label")["task_id"].count().reindex(
    weekly["week_label"], fill_value=0
).values
...
st.markdown(f"*평균 활성률: {avg_activity:.1%}, 평균 반려율: {avg_reject:.1%}*")
